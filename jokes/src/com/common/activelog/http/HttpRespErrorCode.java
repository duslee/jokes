...
public class HttpRespErrorCode extends MPHttpClientData implements
		MPHttpClientResponse {
	public static final String LOGIN_RESP_ERRCODE_SUCCESS = "200";
	// mErrCode = "200" 表示服务器返回成功 否则表示服务器处理失败: 如密码错误等。

	// public static final String LOGIN_RESP_ERRCODE_SUCCESS = "200";
	// public static final int PAY_CHANNEL_NOT_CONFIG = 91404; //no config
	// 无法找到可用支付信息
	...
	public boolean isSuccess() {
		// BaseLog.d("main", "httpRespErroCode.getErCode="+getErrCode());
...
//		BaseLog.d(BaseLog.Tag, "jason:" + jason);
		LogUtils.e("jason:" + jason);
		setErrId(errId);
	...
	public void phraseJason(JSONObject jsonObj) {
		try {
			mErrCode = jsonObj.getString("result");
		} catch (JSONException e) {
	...
	// mErrMessage = jsonObj.getString("msg");
	// } catch (JSONException e) {
	// // TODO Auto-generated catch block
	// e.printStackTrace();
	// }
