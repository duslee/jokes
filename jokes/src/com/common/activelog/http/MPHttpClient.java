...
import com.common.as.network.httpclient.ExecuterServerManager;
	...
	private final Runnable mHttpRunable = new Runnable(){
		...
		public boolean executeOneTime(HttpUriRequest request){
			...
			try{
				...
				BaseLog.d("main", "MPHttpClient.status="+statusId);
				 if (statusId == HttpStatus.SC_OK){
					 ...
					 if (null != tokenH) {
					  ...
					  BaseLog.d("main", "MPHttpClient.errId11="+errId);
					  is = entity.getContent();
					  BaseLog.d("main", "MPHttpClient.is11="+is);
					...
					// clientData = mHttpParser.phraseJason(jason, errId, statusId);
					  ...
					  if (entity != null) {
						  is = entity.getContent();
						  BaseLog.d("main", "MPHttpClient.is="+is);
					}
					  
					  
					 errId = MPHTTP_RESULT_ERR_SERVER;
					 BaseLog.d("main", "MPHttpClient.errId="+errId);
				 }
