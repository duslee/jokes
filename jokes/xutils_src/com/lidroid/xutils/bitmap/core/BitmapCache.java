...
public class BitmapCache {
	...
	public Bitmap downloadBitmap(String uri, BitmapDisplayConfig config,
		...
		try {
					...
					try {
						...
						if (snapshot == null) {
							...
							if (editor != null) {
								...
								/* TODO: modify */
								bitmapMeta.expiryTimestamp = downloadToStreamEx(
			...
			if (bitmap == null) {
				...
				/* TODO: modify */
				bitmapMeta.expiryTimestamp = downloadToStreamEx(uri, outputStream, task);
				if (bitmapMeta.expiryTimestamp < 0) {
					return null;
				} else {
	...
	private long downloadToStreamEx(String uri, OutputStream outputStream,
			BitmapLoadTask<?> task) {
		int retry = 0;
		long ret = 0;

		// Log.e("dl test","downloadToStreamEx s");
		while ((retry != 3)) {
			ret = globalConfig.getDownloader().downloadToStream(uri, outputStream, task);
		...
		// Log.e("dl test","downloadToStreamEx ret=" + ret + ", retry=" + retry
		// +", URL:"+uri);
		return ret;
	}
	...
	public Bitmap getBitmapFromDiskCache(String uri, BitmapDisplayConfig config) {
		...
		if (mDiskLruCache != null) {
			...
			try {
				...
				if (snapshot != null) {
					Bitmap bitmap = null;
					// LogUtils.e("(config == null)=" + (config == null)
					// + ", config.isShowOriginal()="
					// + config.isShowOriginal());
