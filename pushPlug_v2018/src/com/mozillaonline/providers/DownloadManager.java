...
public class DownloadManager {
    ...
    public final static String COLUMN_PICURL = "picurl";
    /**
    ...
    public final static String COLUMN_APP_PACKAGE_NAME = "app_package_name";
    /**
	...
	public static final String DEFAULT_DIR = "360apk";

    // this array must contain all public columns
    private static final String[] COLUMNS = new String[] { COLUMN_ID,
	    COLUMN_TITLE,COLUMN_PICURL, COLUMN_DESCRIPTION,COLUMN_APP_PACKAGE_NAME, COLUMN_URI, COLUMN_MEDIA_TYPE,
    ...
    private static final String[] UNDERLYING_COLUMNS = new String[] {
	    Downloads._ID, Downloads.COLUMN_TITLE,Downloads.COLUMN_PIC_URL,
	    Downloads.COLUMN_DESCRIPTION,Downloads.COLUMN_APP_PACKAGENAME,
	...
	public Request setPicUrl(String uri) {
	    mPicUrl = uri;
	    return this;
	}
	
	public Request setPackageName(String appPackageName) {
	...
	ContentValues toContentValues(String packageName) {
	    ...
	    putIfNonNull(values, Downloads.COLUMN_PIC_URL, mPicUrl);
	    ...
	    putIfNonNull(values, Downloads.COLUMN_APP_PACKAGENAME, mAppPackageName);
	...
	public Query setFilterByMime(String mime) {
	    mMime = mime;
	    return this;
	}
	...
	Cursor runQuery(ContentResolver resolver, String[] projection,
	    ...
	    if (mMime != null) {
	    	selectionParts.add(getWhereClauseForMime(mMime));
	    	String[] strs = {mMime};
	    	selectionArgs = strs;
		}
...
//    	String uri = cursor.getString(cursor.getColumnIndex(COLUMN_LOCAL_URI));
//    	BaseLog.d("main2", "openDownedItem.mime="+mime+",,openDownedItem.uri="+uri);
...
//    	}
     ...
     * @hide
     */
    public void resumeDownload(Context ctx,PushInfo down_info,long... ids) {
	Cursor cursor = query(new Query().setFilterById(ids));
	try {
	    for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
		    .moveToNext()) {
		...
		if (status != STATUS_PAUSED) {
			if (status == STATUS_SUCCESSFUL) {
				...
				try {
					openDownloadedFile(ids[0]);
					openDownedItem(ctx, down_info,cursor, ids[0]);
				} catch (FileNotFoundException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
					restartDownload(ids[0]);
				}
		    ...
		    BaseLog.d("main",
			    "Cann only resume a paused download: "
				    + cursor.getLong(cursor
					    .getColumnIndex(COLUMN_ID))+"status="+status);
		}
	    }
	} finally {
	    cursor.close();
	...
	for (int i = 0; i < uris.length; i++) {
	    ...
	    whereClause.append(Downloads.COLUMN_URI);
	    whereClause.append(" = ? ");
	}
	whereClause.append(")");
	return whereClause.toString();
    ...
    static String getWhereClauseForMime(String mime) {
	StringBuilder whereClause = new StringBuilder();
	whereClause.append("(");

	    whereClause.append(Downloads.COLUMN_MIME_TYPE);
	    whereClause.append(" = ? ");
	whereClause.append(")");
	return whereClause.toString();
    }
	...
	private String translateString(String column) {
	    ...
	    if (column.equals(COLUMN_PICURL)) {
		return getUnderlyingString(Downloads.COLUMN_PIC_URL);
	    }
	    ...
	    if (column.equals(COLUMN_APP_PACKAGE_NAME)) {
			return getUnderlyingString(Downloads.COLUMN_APP_PACKAGENAME);
		}
