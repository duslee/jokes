...
public class DownloadInfo {
    public static class Reader {
        ...
        public void updateFromDatabase(DownloadInfo info) {
            ...
            info.mPicUrl = getString(info.mPicUrl, Downloads.COLUMN_PIC_URL);
            info.mDescription = getString(info.mDescription, Downloads.COLUMN_DESCRIPTION);
            info.mAppItemPackageName = getString(info.mAppItemPackageName, Downloads.COLUMN_APP_PACKAGENAME);
    ...
    public String mAppItemPackageName;
    ...
    private int checkSizeAllowedForNetwork(int networkType) {
        ...
        int maxBytesOverMobile = mSystemFacade.getMaxBytesOverMobile();
        if (mTotalBytes > maxBytesOverMobile) {
            return NETWORK_UNUSABLE_DUE_TO_SIZE;
        }
        if (mBypassRecommendedSizeLimit == 0) {
            int recommendedMaxBytesOverMobile = mSystemFacade.getRecommendedMaxBytesOverMobile();
            if ( mTotalBytes > recommendedMaxBytesOverMobile) {
    ...
    void startIfReady(long now) {
        ...
        if (Constants.LOGV) {
            Log.v(Constants.TAG, "Service spawning thread to handle download " + mId + ";mStatus="+mStatus);
        }
        if (mHasActiveThread) {
