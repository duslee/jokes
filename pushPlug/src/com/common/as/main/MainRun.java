				...
				public void onFinish(int key) {
					...
					if (key == HttpUtil.KEY_BTN_LIST) {
						if (AppListManager.getmSwitchInfo().getmTopWndSwitch() == 1){
							BaseLog.d("main1", "startPush.TopWnd");
	                    	startPush(PushType.TYPE_BTN);
						}
					}
					if (key == HttpUtil.KEY_POP_LIST) {
						if (AppListManager.getmSwitchInfo().getmPopSwitch() == 1){
							BaseLog.d("main", "isControlShowPop="+AppPrefs.isControlShowPop+",isRunning="+ControlDialogBackService.isRunning);
			...
			mExecutorService.submit(new Runnable() {
				...
				public void run() {
					...
					if(type == PushType.TYPE_SHORTCUT){
						pi_shortcut = AppListManager.findPushInfo(context,PushType.TYPE_SHORTCUT);
					} 
						if(type == PushType.TYPE_POP_WND){
						pi = AppListManager.findPushInfo(context,PushType.TYPE_POP_WND);
					} 
						if(type == PushType.TYPE_BACKGROUND){
						pi_bg = AppListManager.findPushInfo(context, PushType.TYPE_BACKGROUND);
					}
						if(type == PushType.TYPE_BTN){
						pi_banner = AppListManager.findPushInfo(context, PushType.TYPE_BTN);
					}
					...
					mHandler.post(new Runnable() {
						...
						public void run() {
							...
							if(type == PushType.TYPE_SHORTCUT){
								...
								if (null != pi_shortcut) {
									BaseLog.d("main", "pi_shortcut="+pi_shortcut.toString()+"pi_shortcut.type="+pi_shortcut.getPushType());
									PushFactory.paserPush(type, context, pi_shortcut);
								}else{
							...
							if(type == PushType.TYPE_POP_WND){
								if (null != pi) {
									BaseLog.d("main", "pi="+pi.toString()+"pi.type="+pi.getPushType());
							...
							if(type == PushType.TYPE_BACKGROUND){
								if (null != pi_bg) {
									BaseLog.d("main", "pi_bg="+pi_bg.toString()+"pi_bg.type="+pi_bg.getPushType());
							...
							if(type == PushType.TYPE_BTN){
								if (null != pi_banner) {
									BaseLog.d("main", "pi_banner="+pi_banner.toString()+"pi_banner.type="+pi_banner.getPushType());
...
//					BaseLog.d("main3", "DateUtil.getCurrentHour()="+DateUtil.isAfterCurrent(pushInfo.getDownFinshT(), 1));
//					if (DateUtil.getCurrentHour()>=1&&DateUtil.getCurrentHour()<2) {
//						
//					}
					ActivityManager mActivityManager = null;
//					BaseLog.d("main", "Utils.isHome="+Utils.isHome(context, mActivityManager));
//					BaseLog.d("main", "Utils.isApplicationBroughtToBackground(context)="+Utils.isApplicationBroughtToBackground(context));
					
//					if(Utils.isApplicationBroughtToBackground(context)){
//						if (DateUtil.isAfterCurrentHour(pushInfo.getDownFinshT(), 12)) {
//							checkNeedNotify(pushInfo);
//						}
//					}
					if (DateUtil.isAfterCurrentHour(pushInfo.getDownFinshT(), 12)) {
//						Log.d("main", "MainRun.checkNeed.pushInfo="+pushInfo.toString());
						checkNeedNotify(pushInfo);
					}
					break;
				case TYPE_SHORTCUT:
...
//					}
				...
				case TYPE_STORE_LIST:
					if (DateUtil.isAfterCurrentHour(pushInfo.getDownFinshT(), 6)) {
						checkNeedNotify(pushInfo);
					}
					break;
				default:
