...
public class AppListBackService extends Service {
	...
	public void onCreate() {
		super.onCreate();
		BaseLog.d("main", "ServiceOncreate");
		homeList = getHomes();
		context = AppListBackService.this;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (mType == TYPE_BG_PUSH) {
			BaseLog.d("main", "mType.newTabView");
			...
			tabView = new TableView(this, new OnTopViewClick() {
				...
				public void onClick(Object userData) {
					...
					PushInfoActionPaser.doClick(AppListBackService.this,
	...
	private void showTopView() {
		...
		if (null == tabView) {
			...
			BaseLog.d("main", "showTopView.TableView");
			tabView = new TableView(this, new OnTopViewClick() {
			...
			BaseLog.d("main", "AppListBackService.showTopView");
			// tabView.fun();
			// showTabView();
	...
	public static void showTabView() {
		...
		if (null != tabView) {
			tabView.fun();
			BaseLog.d("main", "AppListBackService.showTabView");
		} else {
			tabView = new TableView(context, new OnTopViewClick() {

				@Override
				public void onClick(Object userData) {
					Main.showStoreList(context);
				}
			}, null);
			tabView.setIcBmp(BitmapFactory.decodeResource(
					context.getResources(), R.drawable.push_list_icon));
			tabView.fun();
		}
...
//	}
	...
	public boolean isCurrentActivity(String classname) {
		...
		if (!tasks.isEmpty()) {
			ComponentName topActivity = tasks.get(0).topActivity;
			if (topActivity.getShortClassName().equals("." + classname)) {
				return true;
			}
		}
		return false;
