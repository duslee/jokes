...
public class AppFileUtils {
	...
	public static boolean isDirExist(String dir){
		SDCardRoot = Environment.getExternalStorageDirectory().getAbsolutePath();
		File dirFile = new File(SDCardRoot + File.separator + dir + File.separator);
		Log.d("main", "dirfile="+dirFile.getAbsolutePath());
		if (dirFile.exists() ) {
			return true;
		}
		return false;
...
//	}
    ...
    {
        return AppFileUtils.checkStorageRom(Environment.getDataDirectory(), size);
    }

    public static boolean checkSdcardHavEnghStorage(long size)
    {
        return AppFileUtils.checkStorageRom(getExternalStorageDirectory(), size);
    }
